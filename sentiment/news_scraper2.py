...
def scrape(q=None, language=None, country=None, category=None, sources=None, from_param=None, to=None, pageSize=None,
    ...
    for i in every_data:
        ...
        total += 1
        #urls.append(i['url'])
        t_score =sentiment.sentiment_str(i['title'])
        d_score = sentiment.sentiment_str(i['description'])
        date_score = sentiment.sentiment_str(i['from_param'])
        time_score= sentiment.sentiment_str(i['publishedAt'])
        key = time_score[:10]
        print(key)
